<template>
	<div :class="[{
		...
		<div
			...
			v-if="value && !noFlags">
			<div :class="`iti-flag-small iti-flag ${value.toLowerCase()}`"></div>
		</div>
		<input
			:disabled="disabled"
			:id="id"
		...
		<label :class="error ? 'text-danger' : null"
			   :for="id"
			   :style="[colorStyle]"
			   @click="onFocus"
			   class="field-label"
		...
		<Transition name="slide">
			<div class="country-list"
				...
				<div :class="[
					...
					<div class="flag-container" v-if="!noFlags">
						<div :class="`iti-flag-small iti-flag ${item.iso2.toLowerCase()}`"></div>
					</div>
					<div class="dots-text">
						{{ item.name }}
					</div>
...
<script>
	...
	const itemHeight = 40; // if you modify this value, you should update the height property on css (.country-list-item)

	export default {
		name: 'BakePhoneCountrySelector',
		directives: {
		...
		mounted() {
			this.$parent.$on('phone-number-focused', () => {
				this.isFocus = false
			})
		},
		methods: {
