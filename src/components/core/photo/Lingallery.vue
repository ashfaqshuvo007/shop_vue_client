<template>
	<div :style="lingalleryStyle" class="lingallery" @click="clickedLingallery($event)">
		<figure :style="figureStyle" itemprop="associatedMedia">
			<div class="lingallery_spinner">
				<half-circle-spinner :animation-duration="1000" :color="accentColor" :size="60" v-if="isLoading"/>
			</div>
			<img :class="{ loading: isLoading }" :src="currentImage" :style="mainImageStyle"
				 @click="handleLargeImageClick"
				 ref="mainImage" v-swipe="handleImageSwipe">
			<a @click="showPreviousImage" class="control left" v-if="!leftControlClass"><span
				style="position:relative;top:calc(50% - 12px)">&#9664;</span></a>
			<a @click="showNextImage" class="control right" v-if="!rightControlClass"><span
				style="position:relative;top:calc(50% - 12px)">&#9654;</span></a>
			...
			<div class="lingallery_thumbnails_content">
				<div :key="index" class="lingallery_thumbnails_content_elem" v-for="(item, index) in items">
					<img :src="item.thumbnail"
						 :style="thumbnailStyle(index)" height="64"
...
<script>
	...
	export default {
		...
		computed: {
			...
			mainImageStyle() {
				let mainImageStyle = this.imageStyle;
			...
			figureStyle() {
				if (this.currentImage) {
					return this.getImageSize(this.currentImage).then(result => {
						let heightValue = this.mainImageStyle;
		...
		methods: {
			clickedLingallery(event) {
				this.selectedCallback(this.object);
			},
		...
		mounted() {
			if (this.items.length > this.startImage) {
